...
### P
# Estimate the model:
GLM_Freq <- glm(Count ~ 1 + Region + Age + FamilySituation + WinterCoverage +
...
IC_T <- rbind(IC_T, c(AIC(GLM_Freq), BIC(GLM_Freq)))
# Retrieve log-likelihood and number of parameters:
logL_GLM_T <- logLik(GLM_Freq)
K_GLM_T <- length(GLM_Freq$coefficients)

### NB
# Estimate the model:
GLM_Freq <- glm.nb(Count ~ 1 + Region + Age + FamilySituation + WinterCoverage +
                     MoneyCoverage + VehicleCoverage + MedicalCoverage +
                     AccidentCoverage + CancelCoverage +
...
IC_T <- rbind(IC_T, c(AIC(GLM_Freq), BIC(GLM_Freq)))
# Retrieve log-likelihood and number of parameters:
logL_GLM_T <- rbind(logL_GLM_T, logLik(GLM_Freq))
K_GLM_T <- rbind(K_GLM_T, length(GLM_Freq$coefficients))

### ZIP
# Estimate the model:
GLM_Freq <- gam(list(Count ~ 1 + Region + Age + FamilySituation + WinterCoverage +
                       MoneyCoverage + VehicleCoverage + MedicalCoverage +
                       AccidentCoverage + CancelCoverage + 
...
fm <- 1
mal <- Opt_Scores_T[fm, 3]
maxi <- Opt_Scores_T[fm, 2]
start <- Opt_Scores_T[fm, 1]
...
GAM_Freq_T <- gam(Count ~ 1 + Region + Age + FamilySituation + WinterCoverage +
                  ...
                  offset = log(Exposure), data = T_Freq_Train, family = poisson(link = 'log'))
# Plot the spline:
plot(GAM_Freq_T, col = 'blue', xlab = 'Claim score level', 
     ylab = 'Estimated effect')[[1]][c('x', 'se', 'fit')]
# Summarize the model output:
summary(GAM_Freq_T)
# Retrieve the model predictions:
GAM_PG_T_fc <- predict(GAM_Freq_T, T_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_T <- rbind(IC_T, c(AIC(GAM_Freq_T), BIC(GAM_Freq_T)))
# Retrieve log-likelihood and number of parameters:
logL_One_T <- logLik(GAM_Freq_T)
K_One_T <- length(GAM_Freq_T$coefficients)
...
summary(GAM_Freq_T)
# Retrieve the model predictions:
GAM_PIG_T_fc <- predict(GAM_Freq_T, T_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_T <- rbind(IC_T, c(AIC(GAM_Freq_T), BIC(GAM_Freq_T)))
# Retrieve log-likelihood and number of parameters:
logL_One_T <- rbind(logL_One_T, logLik(GAM_Freq_T))
K_One_T <- rbind(K_One_T, length(GAM_Freq_T$coefficients))
...
summary(GAM_Freq_T)
# Retrieve the model predictions:
GAM_PP_T_fc <- predict(GAM_Freq_T, T_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_T <- rbind(IC_T, c(AIC(GAM_Freq_T), BIC(GAM_Freq_T)))
# Retrieve log-likelihood and number of parameters:
logL_One_T <- rbind(logL_One_T, logLik(GAM_Freq_T))
...
summary(GAM_Freq_T)
# Retrieve the model predictions:
GAM_NBG_T_fc <- predict(GAM_Freq_T, T_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_T <- rbind(IC_T, c(AIC(GAM_Freq_T), BIC(GAM_Freq_T)))
# Retrieve log-likelihood and number of parameters:
logL_One_T <- rbind(logL_One_T, logLik(GAM_Freq_T))
...
summary(GAM_Freq_T)
# Retrieve the model predictions:
GAM_NBIG_T_fc <- predict(GAM_Freq_T, T_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_T <- rbind(IC_T, c(AIC(GAM_Freq_T), BIC(GAM_Freq_T)))
# Retrieve log-likelihood and number of parameters:
logL_One_T <- rbind(logL_One_T, logLik(GAM_Freq_T))
...
summary(GAM_Freq_T)
# Retrieve the model predictions:
GAM_NBP_T_fc <- predict(GAM_Freq_T, T_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_T <- rbind(IC_T, c(AIC(GAM_Freq_T), BIC(GAM_Freq_T)))
# Retrieve log-likelihood and number of parameters:
logL_One_T <- rbind(logL_One_T, logLik(GAM_Freq_T))
...
T_Freq$Score <- Score(Psi = mal, s = maxi, l_0 = start, Input_Score = Input_Score_T)
# Split data into training and test set:
T_Freq_Train <- T_Freq[which((as.Date(T_Freq$dt_Eind_risico) <= as.Date(dt_Split)) & (T_Freq$History == 0)), ]
T_Freq_Test <- T_Freq[which((as.Date(T_Freq$dt_Start_risico) >= as.Date(dt_Split)) & (T_Freq$History == 0)), ]
# Estimate the model:
...
summary(GAM_Freq_T)
# Retrieve the model predictions:
GAM_ZIPG_T_fc <- predict(GAM_Freq_T, T_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_T <- rbind(IC_T, c(AIC(GAM_Freq_T), BIC(GAM_Freq_T)))
# Retrieve log-likelihood and number of parameters:
logL_One_T <- rbind(logL_One_T, logLik(GAM_Freq_T))
...
summary(GAM_Freq_T)
# Retrieve the model predictions:
GAM_ZIPIG_T_fc <- predict(GAM_Freq_T, T_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_T <- rbind(IC_T, c(AIC(GAM_Freq_T), BIC(GAM_Freq_T)))
# Retrieve log-likelihood and number of parameters:
logL_One_T <- rbind(logL_One_T, logLik(GAM_Freq_T))
...
summary(GAM_Freq_T)
# Retrieve the model predictions:
GAM_ZIPP_T_fc <- predict(GAM_Freq_T, T_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_T <- rbind(IC_T, c(AIC(GAM_Freq_T), BIC(GAM_Freq_T)))
# Retrieve log-likelihood and number of parameters:
logL_One_T <- rbind(logL_One_T, logLik(GAM_Freq_T))
...
summary(Lin_Freq_T)
# Retrieve the model predictions:
Lin_PG_T_fc <- predict(Lin_Freq_T, T_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_T <- rbind(IC_T, c(AIC(Lin_Freq_T), BIC(Lin_Freq_T)))
# Retrieve log-likelihood and number of parameters:
logL_One_T <- rbind(logL_One_T, logLik(GAM_Freq_T))
K_One_T <- rbind(K_One_T, length(GAM_Freq_T$coefficients))
...
summary(Lin_Freq_T)
# Retrieve the model predictions:
Lin_PIG_T_fc <- predict(Lin_Freq_T, T_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_T <- rbind(IC_T, c(AIC(Lin_Freq_T), BIC(Lin_Freq_T)))
# Retrieve log-likelihood and number of parameters:
logL_One_T <- rbind(logL_One_T, logLik(Lin_Freq_T))
K_One_T <- rbind(K_One_T, length(Lin_Freq_T$coefficients))
...
summary(Lin_Freq_T)
# Retrieve the model predictions:
Lin_PP_T_fc <- predict(Lin_Freq_T, T_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_T <- rbind(IC_T, c(AIC(Lin_Freq_T), BIC(Lin_Freq_T)))
# Retrieve log-likelihood and number of parameters:
logL_One_T <- rbind(logL_One_T, logLik(Lin_Freq_T))
...
summary(Lin_Freq_T)
# Retrieve the model predictions:
Lin_NBG_T_fc <- predict(Lin_Freq_T, T_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_T <- rbind(IC_T, c(AIC(Lin_Freq_T), BIC(Lin_Freq_T)))
# Retrieve log-likelihood and number of parameters:
logL_One_T <- rbind(logL_One_T, logLik(Lin_Freq_T))
...
summary(Lin_Freq_T)
# Retrieve the model predictions:
Lin_NBIG_T_fc <- predict(Lin_Freq_T, T_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_T <- rbind(IC_T, c(AIC(Lin_Freq_T), BIC(Lin_Freq_T)))
# Retrieve log-likelihood and number of parameters:
logL_One_T <- rbind(logL_One_T, logLik(Lin_Freq_T))
...
summary(Lin_Freq_T)
# Retrieve the model predictions:
Lin_NBP_T_fc <- predict(Lin_Freq_T, T_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_T <- rbind(IC_T, c(AIC(Lin_Freq_T), BIC(Lin_Freq_T)))
# Retrieve log-likelihood and number of parameters:
logL_One_T <- rbind(logL_One_T, logLik(Lin_Freq_T))
...
summary(Lin_Freq_T)
# Retrieve the model predictions:
Lin_ZIPG_T_fc <- predict(Lin_Freq_T, T_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_T <- rbind(IC_T, c(AIC(Lin_Freq_T), BIC(Lin_Freq_T)))
# Retrieve log-likelihood and number of parameters:
logL_One_T <- rbind(logL_One_T, logLik(Lin_Freq_T))
...
summary(Lin_Freq_T)
# Retrieve the model predictions:
Lin_ZIPIG_T_fc <- predict(Lin_Freq_T, T_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_T <- rbind(IC_T, c(AIC(Lin_Freq_T), BIC(Lin_Freq_T)))
# Retrieve log-likelihood and number of parameters:
logL_One_T <- rbind(logL_One_T, logLik(Lin_Freq_T))
...
summary(Lin_Freq_T)
# Retrieve the model predictions:
Lin_ZIPP_T_fc <- predict(Lin_Freq_T, T_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_T <- rbind(IC_T, c(AIC(Lin_Freq_T), BIC(Lin_Freq_T)))
# Retrieve log-likelihood and number of parameters:
logL_One_T <- rbind(logL_One_T, logLik(Lin_Freq_T))
...
Mod_fc <- cbind(GLM_PG_T_fc, GLM_PIG_T_fc, GLM_PP_T_fc, 
                GLM_NBG_T_fc, GLM_NBIG_T_fc, GLM_NBP_T_fc,
                GLM_ZIPG_T_fc, GLM_ZIPIG_T_fc, GLM_ZIPP_T_fc,
                GAM_PG_T_fc * GLM_G_T_Sev_fc, GAM_PIG_T_fc * GLM_IG_T_Sev_fc, GAM_PP_T_fc * GLM_P_T_Sev_fc,
                GAM_NBG_T_fc * GLM_G_T_Sev_fc, GAM_NBIG_T_fc * GLM_IG_T_Sev_fc, GAM_NBP_T_fc * GLM_P_T_Sev_fc,
                GAM_ZIPG_T_fc * GLM_G_T_Sev_fc, GAM_ZIPIG_T_fc * GLM_IG_T_Sev_fc, GAM_ZIPP_T_fc * GLM_P_T_Sev_fc,
                Lin_PG_T_fc * GLM_G_T_Sev_fc, Lin_PIG_T_fc * GLM_IG_T_Sev_fc, Lin_PP_T_fc * GLM_P_T_Sev_fc,
                Lin_NBG_T_fc * GLM_G_T_Sev_fc, Lin_NBIG_T_fc * GLM_IG_T_Sev_fc, Lin_NBP_T_fc * GLM_P_T_Sev_fc,
                Lin_ZIPG_T_fc * GLM_G_T_Sev_fc, Lin_ZIPIG_T_fc * GLM_IG_T_Sev_fc, Lin_ZIPP_T_fc * GLM_P_T_Sev_fc)
...
LR_Test_Single_T <- matrix(cbind(rep(NA, 9 * 2), rep(NA, 9 * 2)), 9 * 2, 2)
logL_null_Single_T <- c(rep(rep(logL_GLM_T, each = 3), 2))
logL_alt_Single_T <- c(logL_One_T)
K_null_Single_T <- c(rep(rep(K_GLM_T, each = 3), 2))
K_alt_Single_T <- c(K_One_T)

### Calculation of test statistics and p-values
for (assump in 1:2) {
  for (mod in 1:9) {
    ind <- (assump - 1) * 9 + mod
    LR_Test_Single_T[ind, 1] <- (-2) * (logL_null_Single_T[ind] - logL_alt_Single_T[ind])
    LR_Test_Single_T[ind, 2] <- 1 - pchisq(LR_Test_Single_T[ind, 1], df = (K_alt_Single_T[ind] - K_null_Single_T[ind]))
  }
}
