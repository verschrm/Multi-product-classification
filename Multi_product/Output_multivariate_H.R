...
H_Freq$Score_H <- Score(Psi = mal[3], s = maxi[3], l_0 = start[3], Input_Score = Input_Score_H)
T_Freq$Score <- Score(Psi = mal[4], s = maxi[4], l_0 = start[4], Input_Score = Input_Score_T)

H_Freq$Score_HC <- Multi_Score(Input_Score_A = Input_Score_H, Data_A = H_Freq, Data_B = HC_Freq, l_0_B = start[2])
H_Freq$Score_GL <- Multi_Score(Input_Score_A = Input_Score_H, Data_A = H_Freq, Data_B = GL_Freq, l_0_B = start[1])
H_Freq$Score_T <- Multi_Score(Input_Score_A = Input_Score_H, Data_A = H_Freq, Data_B = T_Freq, l_0_B = start[4])
...
# Plot the splines:
plot(GAM_Freq_H, col = 'blue', xlab = 'Claim score level', ylab = 'Estimated effect', pages = 1)
# Summarize the model output:
summary(GAM_Freq_H)
# Retrieve the model predictions:
GAM_PG_fc <- predict(GAM_Freq_H, H_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_H <- rbind(IC_H, c(AIC(GAM_Freq_H), BIC(GAM_Freq_H)))
# Retrieve log-likelihood and number of parameters:
logL_Multi_H <- logLik(GAM_Freq_H)
K_Multi_H <- length(GAM_Freq_H$coefficients)
...
summary(GAM_Freq_H)
# Retrieve the model predictions:
GAM_PIG_fc <- predict(GAM_Freq_H, H_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_H <- rbind(IC_H, c(AIC(GAM_Freq_H), BIC(GAM_Freq_H)))
# Retrieve log-likelihood and number of parameters:
logL_Multi_H <- rbind(logL_Multi_H, logLik(GAM_Freq_H))
K_Multi_H <- rbind(K_Multi_H, length(GAM_Freq_H$coefficients))
...
summary(GAM_Freq_H)
# Retrieve the model predictions:
GAM_PP_fc <- predict(GAM_Freq_H, H_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_H <- rbind(IC_H, c(AIC(GAM_Freq_H), BIC(GAM_Freq_H)))
# Retrieve log-likelihood and number of parameters:
logL_Multi_H <- rbind(logL_Multi_H, logLik(GAM_Freq_H))
...
summary(GAM_Freq_H)
# Retrieve the model predictions:
GAM_NBG_fc <- predict(GAM_Freq_H, H_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_H <- rbind(IC_H, c(AIC(GAM_Freq_H), BIC(GAM_Freq_H)))
# Retrieve log-likelihood and number of parameters:
logL_Multi_H <- rbind(logL_Multi_H, logLik(GAM_Freq_H))
...
summary(GAM_Freq_H)
# Retrieve the model predictions:
GAM_NBIG_fc <- predict(GAM_Freq_H, H_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_H <- rbind(IC_H, c(AIC(GAM_Freq_H), BIC(GAM_Freq_H)))
# Retrieve log-likelihood and number of parameters:
logL_Multi_H <- rbind(logL_Multi_H, logLik(GAM_Freq_H))
...
summary(GAM_Freq_H)
# Retrieve the model predictions:
GAM_NBP_fc <- predict(GAM_Freq_H, H_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_H <- rbind(IC_H, c(AIC(GAM_Freq_H), BIC(GAM_Freq_H)))
# Retrieve log-likelihood and number of parameters:
logL_Multi_H <- rbind(logL_Multi_H, logLik(GAM_Freq_H))
...
summary(GAM_Freq_H)
# Retrieve the model predictions:
GAM_ZIPG_fc <- predict(GAM_Freq_H, H_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_H <- rbind(IC_H, c(AIC(GAM_Freq_H), BIC(GAM_Freq_H)))
# Retrieve log-likelihood and number of parameters:
logL_Multi_H <- rbind(logL_Multi_H, logLik(GAM_Freq_H))
...
summary(GAM_Freq_H)
# Retrieve the model predictions:
GAM_ZIPIG_fc <- predict(GAM_Freq_H, H_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_H <- rbind(IC_H, c(AIC(GAM_Freq_H), BIC(GAM_Freq_H)))
# Retrieve log-likelihood and number of parameters:
logL_Multi_H <- rbind(logL_Multi_H, logLik(GAM_Freq_H))
...
summary(GAM_Freq_H)
# Retrieve the model predictions:
GAM_ZIPP_fc <- predict(GAM_Freq_H, H_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_H <- rbind(IC_H, c(AIC(GAM_Freq_H), BIC(GAM_Freq_H)))
# Retrieve log-likelihood and number of parameters:
logL_Multi_H <- rbind(logL_Multi_H, logLik(GAM_Freq_H))
...
summary(Lin_Freq_H)
# Retrieve the model predictions:
Lin_PG_fc <- predict(Lin_Freq_H, H_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_H <- rbind(IC_H, c(AIC(Lin_L_H), BIC(Lin_L_H)))
# Retrieve log-likelihood and number of parameters:
logL_Multi_H <- rbind(logL_Multi_H, logLik(Lin_L_H))
K_Multi_H <- rbind(K_Multi_H, length(Lin_L_H$coefficients))
...
summary(Lin_Freq_H)
# Retrieve the model predictions:
Lin_PIG_fc <- predict(Lin_Freq_H, H_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_H <- rbind(IC_H, c(AIC(Lin_L_H), BIC(Lin_L_H)))
# Retrieve log-likelihood and number of parameters:
logL_Multi_H <- rbind(logL_Multi_H, logLik(Lin_L_H))
...
summary(Lin_Freq_H)
# Retrieve the model predictions:
Lin_PP_fc <- predict(Lin_Freq_H, H_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_H <- rbind(IC_H, c(AIC(Lin_L_H), BIC(Lin_L_H)))
# Retrieve log-likelihood and number of parameters:
logL_Multi_H <- rbind(logL_Multi_H, logLik(Lin_L_H))
...
summary(Lin_Freq_H)
# Retrieve the model predictions:
Lin_NBG_fc <- predict(Lin_Freq_H, H_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_H <- rbind(IC_H, c(AIC(Lin_L_H), BIC(Lin_L_H)))
# Retrieve log-likelihood and number of parameters:
logL_Multi_H <- rbind(logL_Multi_H, logLik(Lin_L_H))
...
summary(Lin_Freq_H)
# Retrieve the model predictions:
Lin_NBIG_fc <- predict(Lin_Freq_H, H_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_H <- rbind(IC_H, c(AIC(Lin_L_H), BIC(Lin_L_H)))
# Retrieve log-likelihood and number of parameters:
logL_Multi_H <- rbind(logL_Multi_H, logLik(Lin_L_H))
...
summary(Lin_Freq_H)
# Retrieve the model predictions:
Lin_NBP_fc <- predict(Lin_Freq_H, H_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_H <- rbind(IC_H, c(AIC(Lin_L_H), BIC(Lin_L_H)))
# Retrieve log-likelihood and number of parameters:
logL_Multi_H <- rbind(logL_Multi_H, logLik(Lin_L_H))
...
H_Freq$Score_GL_start <- H_Freq$Score_GL - start[1]
H_Freq$Score_HC_start <- H_Freq$Score_HC - start[2]
H_Freq$Score_H_start <- H_Freq$Score_H - start[3]
H_Freq$Score_T_start <- H_Freq$Score_T - start[4]
...
summary(Lin_Freq_H)
# Retrieve the model predictions:
Lin_ZIPG_fc <- predict(Lin_Freq_H, H_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_H <- rbind(IC_H, c(AIC(Lin_L_H), BIC(Lin_L_H)))
# Retrieve log-likelihood and number of parameters:
logL_Multi_H <- rbind(logL_Multi_H, logLik(Lin_L_H))
...
summary(Lin_Freq_H)
# Retrieve the model predictions:
Lin_ZIPIG_fc <- predict(Lin_Freq_H, H_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_H <- rbind(IC_H, c(AIC(Lin_L_H), BIC(Lin_L_H)))
# Retrieve log-likelihood and number of parameters:
logL_Multi_H <- rbind(logL_Multi_H, logLik(Lin_L_H))
...
summary(Lin_Freq_H)
# Retrieve the model predictions:
Lin_ZIPP_fc <- predict(Lin_Freq_H, H_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_H <- rbind(IC_H, c(AIC(Lin_L_H), BIC(Lin_L_H)))
# Retrieve log-likelihood and number of parameters:
logL_Multi_H <- rbind(logL_Multi_H, logLik(Lin_L_H))
...
Mod_fc <- cbind(Mod_fc, 
                GAM_PG_fc * GLM_G_H_Sev_fc, GAM_PIG_fc * GLM_IG_H_Sev_fc, GAM_PP_fc * GLM_P_H_Sev_fc,
                GAM_NBG_fc * GLM_G_H_Sev_fc, GAM_NBIG_fc * GLM_IG_H_Sev_fc, GAM_NBP_fc * GLM_P_H_Sev_fc,
                GAM_ZIPG_fc * GLM_G_H_Sev_fc, GAM_ZIPIG_fc * GLM_IG_H_Sev_fc, GAM_ZIPP_fc * GLM_P_H_Sev_fc,
                Lin_PG_fc * GLM_G_H_Sev_fc, Lin_PIG_fc * GLM_IG_H_Sev_fc, Lin_PP_fc * GLM_P_H_Sev_fc,
                Lin_NBG_fc * GLM_G_H_Sev_fc, Lin_NBIG_fc * GLM_IG_H_Sev_fc, Lin_NBP_fc * GLM_P_H_Sev_fc,
                Lin_ZIPG_fc * GLM_G_H_Sev_fc, Lin_ZIPIG_fc * GLM_IG_H_Sev_fc, Lin_ZIPP_fc * GLM_P_H_Sev_fc)
...
LR_Test_Multi_H <- matrix(cbind(rep(NA, 9 * 2), rep(NA, 9 * 2)), 9 * 2, 2)
logL_null_Multi_H <- c(logL_One_H)
logL_alt_Multi_H <- c(logL_Multi_H)
K_null_Multi_H <- c(K_One_H)
K_alt_Multi_H <- c(K_Multi_H)

### Calculation of test statistics and p-values
for (assump in 1:2) {
  for (mod in 1:9) {
    ind <- (assump - 1) * 9 + mod
    LR_Test_Multi_H[ind, 1] <- (-2) * (logL_null_Multi_H[ind] - logL_alt_Multi_H[ind])
    LR_Test_Multi_H[ind, 2] <- 1 - pchisq(LR_Test_Multi_H[ind, 1], df = (K_alt_Multi_H[ind] - K_null_Multi_H[ind]))
  }
}
