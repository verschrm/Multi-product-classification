...
# Estimate the model:
PL_Freq_T <- gam(Count ~ 1 + Region + Age + FamilySituation + WinterCoverage +
...
# Plot the splines:
plot(PL_Freq_T, col = 'blue', xlab = 'Claim score level', ylab = 'Estimated effect', pages = 1)
# Summarize the model output:
summary(PL_Freq_T)
# Retrieve the model predictions:
PL_PG_fc <- predict(PL_Freq_T, T_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_T <- rbind(IC_T, c(AIC(PL_Freq_T), BIC(PL_Freq_T)))
...
summary(PL_Freq_T)
# Retrieve the model predictions:
PL_PIG_fc <- predict(PL_Freq_T, T_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_T <- rbind(IC_T, c(AIC(PL_Freq_T), BIC(PL_Freq_T)))
...
summary(PL_Freq_T)
# Retrieve the model predictions:
PL_PP_fc <- predict(PL_Freq_T, T_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_T <- rbind(IC_T, c(AIC(PL_Freq_T), BIC(PL_Freq_T)))
...
summary(PL_Freq_T)
# Retrieve the model predictions:
PL_NBG_fc <- predict(PL_Freq_T, T_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_T <- rbind(IC_T, c(AIC(PL_Freq_T), BIC(PL_Freq_T)))
...
summary(PL_Freq_T)
# Retrieve the model predictions:
PL_NBIG_fc <- predict(PL_Freq_T, T_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_T <- rbind(IC_T, c(AIC(PL_Freq_T), BIC(PL_Freq_T)))
...
summary(PL_Freq_T)
# Retrieve the model predictions:
PL_NBP_fc <- predict(PL_Freq_T, T_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_T <- rbind(IC_T, c(AIC(PL_Freq_T), BIC(PL_Freq_T)))
...
T_Freq_Test <- T_Freq[which((as.Date(T_Freq$dt_Start) >= as.Date(dt_Split)) & (T_Freq$History == 0)), ]
# Estimate the model:
PL_Freq_T <- gam(list(Count ~ 1 + Region + Age + FamilySituation + WinterCoverage +
                        MoneyCoverage + VehicleCoverage + MedicalCoverage +
                        AccidentCoverage + CancelCoverage +
                        s(Score_GL, bs = 'ps', k = k_cr, m = 0, fx = TRUE, pc = start[1]) +
                        s(Score_HC, bs = 'ps', k = k_cr, m = 0, fx = TRUE, pc = start[2]) +
...
summary(PL_Freq_T)
# Retrieve the model predictions:
PL_ZIPG_fc <- predict(PL_Freq_T, T_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_T <- rbind(IC_T, c(AIC(PL_Freq_T), BIC(PL_Freq_T)))
...
summary(PL_Freq_T)
# Retrieve the model predictions:
PL_ZIPIG_fc <- predict(PL_Freq_T, T_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_T <- rbind(IC_T, c(AIC(PL_Freq_T), BIC(PL_Freq_T)))
...
summary(PL_Freq_T)
# Retrieve the model predictions:
PL_ZIPP_fc <- predict(PL_Freq_T, T_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_T <- rbind(IC_T, c(AIC(PL_Freq_T), BIC(PL_Freq_T)))
...
Mod_fc <- cbind(Mod_fc, 
                PL_PG_fc * GLM_G_T_Sev_fc, PL_PIG_fc * GLM_IG_T_Sev_fc, PL_PP_fc * GLM_P_T_Sev_fc,
                PL_NBG_fc * GLM_G_T_Sev_fc, PL_NBIG_fc * GLM_IG_T_Sev_fc, PL_NBP_fc * GLM_P_T_Sev_fc,
                PL_ZIPG_fc * GLM_G_T_Sev_fc, PL_ZIPIG_fc * GLM_IG_T_Sev_fc, PL_ZIPP_fc * GLM_P_T_Sev_fc)
