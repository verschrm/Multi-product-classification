...
# Estimate the model:
PL_Freq_H <- gam(Count ~ 1 + ProductType + Year + Age +
...
# Plot the splines:
plot(PL_Freq_H, col = 'blue', xlab = 'Claim score level', ylab = 'Estimated effect', pages = 1)
# Summarize the model output:
summary(PL_Freq_H)
# Retrieve the model predictions:
PL_PG_fc <- predict(PL_Freq_H, H_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_H <- rbind(IC_H, c(AIC(PL_Freq_H), BIC(PL_Freq_H)))
...
summary(PL_Freq_H)
# Retrieve the model predictions:
PL_PIG_fc <- predict(PL_Freq_H, H_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_H <- rbind(IC_H, c(AIC(PL_Freq_H), BIC(PL_Freq_H)))
...
summary(PL_Freq_H)
# Retrieve the model predictions:
PL_PP_fc <- predict(PL_Freq_H, H_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_H <- rbind(IC_H, c(AIC(PL_Freq_H), BIC(PL_Freq_H)))
...
summary(PL_Freq_H)
# Retrieve the model predictions:
PL_NBG_fc <- predict(PL_Freq_H, H_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_H <- rbind(IC_H, c(AIC(PL_Freq_H), BIC(PL_Freq_H)))
...
summary(PL_Freq_H)
# Retrieve the model predictions:
PL_NBIG_fc <- predict(PL_Freq_H, H_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_H <- rbind(IC_H, c(AIC(PL_Freq_H), BIC(PL_Freq_H)))
...
summary(PL_Freq_H)
# Retrieve the model predictions:
PL_NBP_fc <- predict(PL_Freq_H, H_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_H <- rbind(IC_H, c(AIC(PL_Freq_H), BIC(PL_Freq_H)))
...
H_Freq_Test <- H_Freq[which((as.Date(H_Freq$dt_Start_risico) >= as.Date(dt_Split)) & (H_Freq$History == 0)), ]
# Estimate the model:
PL_Freq_H <- gam(list(Count ~ 1 + ProductType + Year + Age +
                        FamilySituation + BuildingType + RoofType + Capacity + ConstructionYear + Residence +
                        Urban + GlassCoverage +
                        s(Score_GL, bs = 'ps', k = k_cr, m = 0, fx = TRUE, pc = start[1]) +
                        s(Score_HC, bs = 'ps', k = k_cr, m = 0, fx = TRUE, pc = start[2]) +
...
summary(PL_Freq_H)
# Retrieve the model predictions:
PL_ZIPG_fc <- predict(PL_Freq_H, H_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_H <- rbind(IC_H, c(AIC(PL_Freq_H), BIC(PL_Freq_H)))
...
summary(PL_Freq_H)
# Retrieve the model predictions:
PL_ZIPIG_fc <- predict(PL_Freq_H, H_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_H <- rbind(IC_H, c(AIC(PL_Freq_H), BIC(PL_Freq_H)))
...
summary(PL_Freq_H)
# Retrieve the model predictions:
PL_ZIPP_fc <- predict(PL_Freq_H, H_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_H <- rbind(IC_H, c(AIC(PL_Freq_H), BIC(PL_Freq_H)))
...
Mod_fc <- cbind(Mod_fc, 
                PL_PG_fc * GLM_G_H_Sev_fc, PL_PIG_fc * GLM_IG_H_Sev_fc, PL_PP_fc * GLM_P_H_Sev_fc,
                PL_NBG_fc * GLM_G_H_Sev_fc, PL_NBIG_fc * GLM_IG_H_Sev_fc, PL_NBP_fc * GLM_P_H_Sev_fc,
                PL_ZIPG_fc * GLM_G_H_Sev_fc, PL_ZIPIG_fc * GLM_IG_H_Sev_fc, PL_ZIPP_fc * GLM_P_H_Sev_fc)
