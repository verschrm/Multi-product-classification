...
# Estimate the model:
PL_Freq_HC <- gam(Count ~ 1 + ProductType + Year + Age + 
...
# Plot the splines:
plot(PL_Freq_HC, col = 'blue', xlab = 'Claim score level', ylab = 'Estimated effect', pages = 1)
# Summarize the model output:
summary(PL_Freq_HC)
# Retrieve the model predictions:
PL_PG_fc <- predict(PL_Freq_HC, HC_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_HC <- rbind(IC_HC, c(AIC(PL_Freq_HC), BIC(PL_Freq_HC)))
...
summary(PL_Freq_HC)
# Retrieve the model predictions:
PL_PIG_fc <- predict(PL_Freq_HC, HC_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_HC <- rbind(IC_HC, c(AIC(PL_Freq_HC), BIC(PL_Freq_HC)))
...
summary(PL_Freq_HC)
# Retrieve the model predictions:
PL_PP_fc <- predict(PL_Freq_HC, HC_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_HC <- rbind(IC_HC, c(AIC(PL_Freq_HC), BIC(PL_Freq_HC)))
...
summary(PL_Freq_HC)
# Retrieve the model predictions:
PL_NBG_fc <- predict(PL_Freq_HC, HC_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_HC <- rbind(IC_HC, c(AIC(PL_Freq_HC), BIC(PL_Freq_HC)))
...
summary(PL_Freq_HC)
# Retrieve the model predictions:
PL_NBIG_fc <- predict(PL_Freq_HC, HC_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_HC <- rbind(IC_HC, c(AIC(PL_Freq_HC), BIC(PL_Freq_HC)))
...
summary(PL_Freq_HC)
# Retrieve the model predictions:
PL_NBP_fc <- predict(PL_Freq_HC, HC_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_HC <- rbind(IC_HC, c(AIC(PL_Freq_HC), BIC(PL_Freq_HC)))
...
HC_Freq_Test <- HC_Freq[which((as.Date(HC_Freq$dt_Start) >= as.Date(dt_Split)) & (HC_Freq$History == 0)), ]
# Estimate the model:
PL_Freq_HC <- gam(list(Count ~ 1 + ProductType + Year + Age + 
                         BuildingType + RoofType + FloorSpace + HomeOwner + Residence + 
                         Urban + GlassCoverage +
                         s(Score_GL, bs = 'ps', k = k_cr, m = 0, fx = TRUE, pc = start[1]) +
                         s(Score_HC, bs = 'ps', k = k_cr, m = 0, fx = TRUE, pc = start[2]) +
...
summary(PL_Freq_HC)
# Retrieve the model predictions:
PL_ZIPG_fc <- predict(PL_Freq_HC, HC_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_HC <- rbind(IC_HC, c(AIC(PL_Freq_HC), BIC(PL_Freq_HC)))
...
summary(PL_Freq_HC)
# Retrieve the model predictions:
PL_ZIPIG_fc <- predict(PL_Freq_HC, HC_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_HC <- rbind(IC_HC, c(AIC(PL_Freq_HC), BIC(PL_Freq_HC)))
...
summary(PL_Freq_HC)
# Retrieve the model predictions:
PL_ZIPP_fc <- predict(PL_Freq_HC, HC_Freq_Test, type = 'response')
# Determine AIC and BIC:
IC_HC <- rbind(IC_HC, c(AIC(PL_Freq_HC), BIC(PL_Freq_HC)))
...
Mod_fc <- cbind(Mod_fc, 
                PL_PG_fc * GLM_G_HC_Sev_fc, PL_PIG_fc * GLM_IG_HC_Sev_fc, PL_PP_fc * GLM_P_HC_Sev_fc,
                PL_NBG_fc * GLM_G_HC_Sev_fc, PL_NBIG_fc * GLM_IG_HC_Sev_fc, PL_NBP_fc * GLM_P_HC_Sev_fc,
                PL_ZIPG_fc * GLM_G_HC_Sev_fc, PL_ZIPIG_fc * GLM_IG_HC_Sev_fc, PL_ZIPP_fc * GLM_P_HC_Sev_fc)
